...
class JsonConsumer(AsyncJsonWebsocketConsumer):
    ...
    # async def accept(self, subprotocol=None, group=None):
    #     """
    #     Accepts an incoming socket
    #     """
    #     await super().send({"type": "websocket.accept", "subprotocol": subprotocol, "group": group})
    ...
    async def receive(self, text_data=None, bytes_data=None, **kwargs):  # 消息接收器
        ...
        # 向组发送消息
        # 接受前端发送的消息,然后传给发送器
        await self.channel_layer.group_send(self.group, {'type': 'json.message', 'data': text_data})
        # type消息类型(决定把消息给那个函数), 会把结果返给对应类型名字的函数, 即不同的type使用不同的发送器 名字中的.会被替换为_

    # 'type': 'group.message' 的消息发送器  # event 消息接收器接收到的消息
    async def json_message(self, event):
        print('group_message', event)
        data = json.loads(event['data'])  # send_json是把数据json话, 而不是需要传入json格式数据
