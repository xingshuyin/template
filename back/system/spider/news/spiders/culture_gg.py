...
class CultureSpider(scrapy.Spider):
    name = 'culture_gg'
    allowed_domains = ['*']
    start_urls = ['https://zwgk.mct.gov.cn/zfxxgkml/503/507/index_3081.html']
    pages = 1
    ...
    def start_requests(self):
        ...
        yield scrapy.Request(url='https://zwgk.mct.gov.cn/zfxxgkml/503/507/index_3081.html', method="GET", callback=self.get_page, dont_filter=True)

    def get_page(self, response):
        page = response.css('*').re(f"var countPage = (\d+)")
        for i in range(int(page[0])):
            url = f"https://zwgk.mct.gov.cn/zfxxgkml/503/507/index_3081{'_'+str(i) if i>0 else ''}.html"
