...
class CultureSpider(scrapy.Spider):
    name = 'culture_hb_mygz'
    allowed_domains = ['*']
    start_urls = ['http://www.hebeitour.gov.cn/xwzx/mtgz/index.html']
    pages = 1
    ...
    def start_requests(self):
        ...
        yield scrapy.Request(url='http://www.hebeitour.gov.cn/xwzx/mtgz/index.html', method="GET", callback=self.get_page, dont_filter=True)
    ...
    def get_page(self, response):
        ...
        for i in range(1, int(page[0]) + 1):
            if i > 3:
                url = f"https://www.hebeitour.gov.cn/zwf/ui/catalog/15932/pc/index_{str(i)}.html"
            else:
                url = f"http://www.hebeitour.gov.cn/xwzx/mtgz/index{'_'+str(i) if i>1 else ''}.html"
